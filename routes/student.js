...
// Returns exams open to this student's course+semester, or exams with no restriction
router.get('/exams', requireStudent, async (req, res) => {
  const { course, branch, semester } = req.session.user;
  // course stored as 'Btech', branch as 'CSE'
  // exam_course_map stores course='Btech', semester='4' etc.
  try {
    const [rows] = await db.execute(
      `SELECT DISTINCT e.exam_id, e.exam_name, e.exam_code,
              e.exam_date, e.reg_deadline AS last_date,
              e.fee_per_subject AS fee, e.mode AS exam_type,
              e.status
       FROM exams e
       LEFT JOIN exam_course_map ecm ON ecm.exam_id = e.exam_id
       WHERE e.status = 'Open'
...
// ─── GET /api/student/exams/:examId/subjects ─────────────────────────────────
router.get('/exams/:examId/subjects', requireStudent, async (req, res) => {
  try {
    const [rows] = await db.execute(
      `SELECT s.subject_id, s.subject_code, s.subject_name, s.department, s.credits
       FROM subjects s
       JOIN exam_subjects es ON s.subject_id = es.subject_id
       WHERE es.exam_id = ?`,
      [req.params.examId]
    ...
    console.error('GET /exams/:id/subjects error:', err);
    res.status(500).json({ success: false, message: err.message });
  }
});

// ─── POST /api/student/register ──────────────────────────────────────────────
// Registers student for an exam + chosen subjects using the stored procedure
router.post('/register', requireStudent, async (req, res) => {
  const { exam_id, subject_ids } = req.body;
  const student_id = req.session.user.id;

  if (!exam_id || !subject_ids || !subject_ids.length)
    return res.status(400).json({ success: false, message: 'exam_id and subject_ids required.' });
...
router.get('/my-registrations', requireStudent, async (req, res) => {
  try {
    const [rows] = await db.execute(
      `SELECT r.reg_id, e.exam_name, e.exam_code,
...
router.get('/hall-tickets', requireStudent, async (req, res) => {
  try {
    const [rows] = await db.execute(
      `SELECT ht.ticket_id, ht.roll_number, ht.seat_number, ht.issued_on, ht.status,
...
router.get('/schedule', requireStudent, async (req, res) => {
  try {
    const [rows] = await db.execute(
      `SELECT es.schedule_id, es.event_name, es.event_date, es.event_status,
...
router.get('/profile', requireStudent, async (req, res) => {
  try {
    const [rows] = await db.execute(
      `SELECT student_id, full_name, email, username,
              course, branch, semester, prn, phone
